...
def _build_tf_prompt(topic: str) -> str:
    ...
    return (
        ...
        f"🚨🚨🚨 MANDATORY FORMAT — READ FIRST 🚨🚨🚨\n"
        f"EVERY single question you write MUST start with one of these 4 exact "
        f"Bengali phrases — no exceptions, no plain/normal MCQ questions "
        f"allowed AT ALL:\n"
        f"1. নিচের কোনটিকে সত্য বললে ভুল হবে না?\n"
        f"2. নিচের কোনটিকে সত্য বললে ভুল হবে?\n"
        f"3. নিচের কোনটিকে মিথ্যা বললে ভুল হবে?\n"
        f"4. নিচের কোনটিকে মিথ্যা বললে ভুল হবে না?\n"
        f"❌ WRONG (never do this): \"কোষচক্রে ইন্টারফেজ দশাকে প্রধানত কী দশা "
        f"হিসেবে গণ্য করা হয়?\" — this is a plain MCQ question, NOT allowed.\n"
        f"✅ RIGHT (do this instead, same fact re-angled): \"নিচের কোনটিকে সত্য "
        f"বললে ভুল হবে না? [ইন্টারফেজ দশাকে যা বলা হয় তার একটি সঠিক বিকল্প + "
        f"৩টি ভুল বিকল্প]\"\n"
        f"Before outputting each question, check: does it literally start with "
        f"one of the 4 phrases above? If not, rewrite it until it does.\n\n"
        ...
        f"═══════════════════════════════\n"
        f"💥 QUESTION TYPE — randomly mix the 4 mandatory patterns above, "
        f"don't repeat one pattern back to back. The correct answer mapping "
        f"for each is EXACT, never map it wrong:\n"
        ...
        f"🚨 FINAL REMINDER: every \"question\" field in your JSON output MUST "
        f"literally begin with one of the 4 mandatory phrases listed at the "
        f"top. A plain/normal question (like \"...কী?\" or \"...কোনটি?\" without "
        f"the সত্য/মিথ্যা...বললে ভুল হবে framing) is INVALID output — rewrite "
        f"it before returning.\n\n"
