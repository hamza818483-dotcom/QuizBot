...
def _build_bangla_prompt(topic: str) -> str:
    ...
    Inherits /pdf's battle-tested quality rules (exact term fidelity, distractor
    quality, two-part explanation, forbidden source-reference phrases, language
    detection, exp_bbox) and adds ONE specialization on top: maximum source
    utilization — every line/portion of the page must produce an MCQ, and
    there is NO maximum MCQ cap (unlike /pdf's default ~15-35 target).
    """
    return (
        ...
        f"MCQ from it, covering different angles (direct fact, definition, "
        f"cause-effect, fill-in-the-blank, comparison, 'কোনটি সঠিক নয়' style).\n"
        f"- NO MAXIMUM MCQ COUNT — there is no upper limit or cap, unlike a "
        f"normal target of ~15-35. Use MAXIMUM possible source utilization: "
        f"squeeze out every usable fact on the page into its own MCQ. More "
        f"content covered = more MCQs expected. Never artificially stop early "
        f"to hit some round number — the page's actual content volume is the "
        f"only limit.\n"
        f"- Never skip a line/portion for being 'too short' or 'minor' — even a "
        f"short one-sentence fact must produce a real MCQ.\n"
        f"- Pay special attention to the LAST lines / bottom of the page and "
        f"footnotes — these are the most commonly missed.\n"
        f"- Never make junk/filler MCQs just to inflate the count — every MCQ "
        f"must be a genuinely distinct, real fact from the page, not a copy-"
        f"paste with cosmetic wording changes.\n"
        f"- NEVER generate MCQs from topic names, chapter titles, headlines, or "
        f"page numbers — these are structural labels, not content.\n"
        f"- Among the MCQs generated, include several that mix several distinct "
        f"facts from the page into a single question — e.g. options that are "
        f"each a combination of 2-3 facts, where only one option has ALL facts "
        f"correct. Keep these moderate difficulty (not extreme/confusing).\n\n"
        ...
        f"- Tables/charts/ছক get special priority — use EVERY cell of information "
        f"in them for MCQs, walk through box-by-box, don't just describe the "
        f"table as a whole.\n\n"
        ...
        f"pattern back to back — this reads as boring/repetitive. Use varied "
        f"question structures: direct fact, definition, cause-effect, comparison, "
        f"fill-in-the-blank, \"কোনটি সঠিক নয়\" style, etc. Mix them naturally.\n"
        f"Occasionally reverse the direction of a fact-based question where the "
        f"same fact supports it (e.g. 'X এর রাজধানী কী?' and elsewhere 'Y কোন "
        f"দেশের রাজধানী?') — don't do this for every fact, just mix it in "
        f"naturally so the set isn't monotonous.\n\n"

        f"═══════════════════════════════\n"
        f"💥 প্রশ্ন (QUESTION)\n"
        f"═══════════════════════════════\n"
        f"- Short and clear (roughly 1-1.5-2 lines), never needlessly wordy.\n"
        f"- Cover every plausible angle the source content could be tested on, "
        f"but never invent facts not present in the source image.\n"
        f"- 🚨 EXACT TERM FIDELITY (CRITICAL): Copy every proper noun, name, "
        f"place, country, term, or spelling EXACTLY as printed on the page — "
        f"character for character. Never substitute a similar-looking or "
        f"similar-sounding word. Before finalizing each question/option/"
        f"explanation, re-check every proper noun against the source image's "
        f"actual spelling.\n\n"
        ...
        f"(pulled from nearby/related information in the same source), so a "
        f"student actually has to know the material rather than guess by "
        f"elimination.\n"
        ...
        f"- Double-check that more than one option can never be defended as "
        f"correct.\n"
        f"- Across the full set, vary WHICH option letter is correct — don't let "
...
def _build_mcq_prompt(topic: str, count) -> str:
    ...
    return (
        ...
        f"🟥 বক্স/ছক COVERAGE\n"
        f"If the page has boxed/ছক/সারণি/info-card content: MCQ from EVERY box "
        f"(skip at most 2-3 truly trivial/empty ones). Dense boxes get MORE THAN "
        f"ONE MCQ (up to 15 if very information-dense). Go box-by-box, don't just "
        f"cover the first few.\n\n"

        f"🟦 QUESTION-ANGLE VARIATION\n"
        f"Don't repeat the same question pattern back-to-back (e.g. always \"X "
        f"সম্পর্কে কোনটি সঠিক?\"). Mix styles: direct fact, definition, cause-effect, "
        f"comparison, fill-in-blank, \"কোনটি সঠিক নয়\". Occasionally reverse a fact's "
        f"direction (e.g. capital→country and country→capital) where it makes "
        f"sense, without overdoing it.\n\n"

        f"💥 প্রশ্ন (QUESTION)\n"
        f"- Short/clear (~1-2 lines), cover every plausible angle, never invent facts.\n"
        f"- Not unnecessarily hard/tricky — clarity over difficulty.\n"
        f"- 🚨 EXACT TERM FIDELITY: copy every proper noun/name/place/term/spelling "
        f"EXACTLY as printed — character for character, never a similar-sounding "
        f"substitute. Re-verify each proper noun against the source image, not memory.\n\n"

        f"💥 অপশন (OPTIONS) — exactly 4\n"
        f"- Generally short (single word/term), ~20% can be a short phrase — don't "
        f"force uniform length.\n"
        f"- All 4 must be real substantive content — never 'yes/no/true/false' filler.\n"
        ...
        f"- Vary which letter is correct across the set — no forced pattern.\n\n"
        ...
        f"🚫 FORBIDDEN SOURCE-REFERENCE WORDS (question AND explanation)\n"
        f"Never refer to the source itself instead of stating facts directly. "
        f"WORD-ROOT ban (any inflected/suffixed form counts): পৃষ্ঠা, চিত্র, বক্স/box, "
        f"ছক/table, সারণি, টেক্সট/text, অনুচ্ছেদ, প্যাসেজ/passage, উদ্দীপক, টপিক/topic — "
...
async def handle_start(msg: dict):
    ...
    if is_auth:
        admin_md = (
            ...
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/img Physics 5` | Image reply → MCQ poll |\n"
            "| `/pdfc` | Multiple image → PDF |\n"
            "| `/done` | Image collection শেষ |\n\n"
            "## 📝 Text/CSV\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/txt` | Text reply → MCQ poll |\n"
            "| `/csv` | CSV reply → channel poll |\n"
            "| `/csvS` | CSV reply → sequential poll |\n\n"
            "## 📚 Q-Bank → PDF\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/qpdf` | chorcha.net mhtml/html → Premium Q&A PDF |\n\n"
            "## 🚀 Rapid Fire (scheduled, comment-based)\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/rapid [topic]` | CSV reply → schedule (channel + local time বেছে নিলে প্রতি 10s এ প্রশ্ন, 12s পর উত্তর reveal) |\n\n"
            "## 🎯 Live Quiz\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/live [topic]` | CSV reply → Live Quiz শুরু |\n"
            "| `/livetime [sec]` | প্রতি প্রশ্নের সময় |\n\n"
            "## ⚙️ Settings\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            ...
            "| --- | --- |\n"
            "| `/bm` | Bookmark PDF |\n"
            "| `/bmexam` | Bookmark MCQ → Quiz |\n\n"
            "## 🧩 D1 Quiz System\n"
            "| Command | কাজ |\n"
            "| --- | --- |\n"
            "| `/q [name]` | CSV → quiz তৈরি |\n"
            "| `/qlist` | সব quiz |\n"
            "| `/qdel [id]` | Quiz delete |\n"
            "| `/pre [quiz_id]` | Quiz preview image set |\n"
            "| `/info [quiz_id]` | Quiz details |\n"
            "| `/send [quiz_id]` | Quiz share → channel |\n"
            "| `/collect` | Poll collect mode on |\n"
            "| `/merge` | Collected polls merge |\n"
            "| `/convert [quiz_id]` | Quiz → CSV export |\n\n"
            "---\n"
            "🚀 **ATLAS BOT** — Atlascourses.com"
        )
        admin_plain_fallback = (
            ...
            "📸 <b>Image</b>\n"
            "<code>/img     </code> Image reply → MCQ poll (e.g. <code>/img Physics 5</code>)\n"
            "<code>/pdfc    </code> Multiple image → PDF\n"
            "<code>/done    </code> Image collection শেষ\n\n"
            "📝 <b>Text/CSV</b>\n"
            "<code>/txt     </code> Text reply → MCQ poll\n"
            "<code>/csv     </code> CSV reply → channel poll\n"
            "<code>/csvS    </code> CSV reply → sequential poll\n\n"
            "📚 <b>Q-Bank → PDF</b>\n"
            "<code>/qpdf    </code> chorcha.net mhtml/html → Premium Q&A PDF\n\n"
            "🚀 <b>Rapid Fire</b> (scheduled, comment-based)\n"
            "<code>/rapid [topic]</code> CSV reply → schedule\n"
            "          Channel + local time বেছে নিলে প্রতি 10s এ প্রশ্ন,\n"
            "          12s পর উত্তর reveal\n\n"
            "🎯 <b>Live Quiz</b>\n"
            ...
            "🔖 <b>Bookmark</b>\n"
            "<code>/bm      </code> Bookmark PDF\n"
            "<code>/bmexam  </code> Bookmark MCQ → Quiz\n\n"
            "🧩 <b>D1 Quiz System</b>\n"
            "<code>/q [name]     </code> CSV → quiz তৈরি\n"
            "<code>/qlist        </code> সব quiz\n"
            "<code>/qdel [id]    </code> Quiz delete\n"
            "<code>/pre [quiz_id]</code> Quiz preview image set\n"
            "<code>/info [quiz_id]</code> Quiz details\n"
            "<code>/send [quiz_id]</code> Quiz share → channel\n"
            "<code>/collect      </code> Poll collect mode on\n"
            "<code>/merge        </code> Collected polls merge\n"
            "<code>/convert [quiz_id]</code> Quiz → CSV export\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "🚀 <b>ATLAS BOT</b> — Atlascourses.com"
        )
        await send_rich_msg(chat_id, admin_md, fallback_text=admin_plain_fallback)
    else:
...
EXTRACTION:
- Extract all MCQs: Bangla/English/mixed, any font (printed/handwritten/bold/italic), blurry/rotated/low-quality images
- Do 3+ independent internal read-throughs, cross-check before finalizing — don't miss the LAST MCQ on page/column. Count visible MCQs, verify your list length matches.
- Strip only numbering prefixes; keep original wording; fix obvious spelling errors without changing meaning

COMPLETENESS (mandatory, zero-tolerance):
- Never output partial/truncated question or option (e.g. missing parenthetical terms, cut-off clauses). A question ends only at proper punctuation (?।) or a clearly finished clause.
- Before finalizing each MCQ: read question start-to-end fully; compare word/segment count vs source — fewer means truncated, re-read; do the same full check for all 4 options and any answer-key text
- Verify spelling character-by-character against source; a misspelled word is as bad as a truncated one
- If a word/name is hard to read: never leave blank/partial — use context, partial visible letters, parenthetical terms, or general knowledge to output the single best, correctly-spelled, complete word
- If question text is unclear but 4 options are readable: infer the question type from the options (e.g. all country names → "কোন দেশের...?" type; all dates → "কত সালে/কবে" type) plus general knowledge, output the best complete question matching those options
- Same completeness+spelling strictness applies to question, all 4 options, and explanation — always

ANSWER DETECTION (triple-check before finalizing):
Answer must trace to an actual source on the page — never guess. Check in this priority order:
A) Mark directly on an option (circle/tick✓/cross✗-elimination/underline/bold/highlight/star) — if ANY option has ANY visual mark, that IS the answer, 100%, no second-guessing, overrides all other sources
B) Answer given right after the MCQ block, before next question
C) Answer table at bottom of same page (e.g. "1-A, 2-C...") — only if no mark on any option
D) Combined answer key several pages later (scan forward through all pages — banks often group answers after 2-3 pages or at document end) — only if no mark on any option
E) Answer key on page(s) immediately before/after, any format — only if no mark on any option
Priority is absolute: A > B > C > D > E. Match strictly by question number (or exact text if numbers unclear).
Only if genuinely no answer found anywhere after scanning all sources → set answer "A" and note in explanation "Answer not found in source" (last resort only).
Convert source format (number/checkmark/circled letter/bold) to standard A/B/C/D. Re-verify each answer twice before finalizing — wrong is worse than missing.

OPTION ORDER (absolute, never reorder/shuffle):
- Whatever label system the source uses (A,B,C,D / a,b,c,d / ক,খ,গ,ঘ / ১,২,৩,৪ / bullets/no label, top-to-bottom or left-to-right) — output uses the SAME VISUAL POSITION order: source's 1st option → output A, 2nd → B, 3rd → C, 4th → D. This is position matching, not label matching.
- Never reorder/sort option text (alphabetically, by value, or any way) — preserve source sequence exactly
- Correct position matching auto-gives correct answer letter, since answer = position of correct text in OUTPUT, not source's original label. Example: source order গ,খ,ক,ঘ, correct is source's "ক" → output slot C (3rd position) → answer="C", not "A"
- Before finalizing each MCQ, verify in 3 steps: (1) output's 4 slots match source's 4 positions, (2) find which slot the correct answer text landed in, (3) confirm answer letter matches that slot — fix any mismatch
- Numbers/years/dates (Bengali বা English digits) stay exactly as in source — never convert between Bengali/English numerals. Verify digit-by-digit (৯↔9, ৬↔6 mixups strictly forbidden).
...
2) Else if no direct explanation but page has other relevant info (paragraph/note/box/table/fact) about this MCQ's topic → build explanation from that info as direct fact (see forbidden phrases below). Max 165 chars, Bengali.
3) Else if nothing relevant exists anywhere → generate the best accurate explanation from your own knowledge. Max 165 chars, Bengali.
Whichever case applies, explanation should convey why the correct option is right + brief context on wrong options — except case 1, used exactly as-is regardless of length/coverage.
Case 1 is always checked FIRST for every MCQ before considering self-generation.

ALL-4-OPTIONS COVERAGE (strict):
- If source has an explanation (case 1) → copy 100% verbatim regardless of which/how many options it covers, never edit/extend
- If source has NO explanation (case 2/3) → you must write one covering all 4 options: why correct is right + why each of the 3 wrong ones is incorrect, within 165 Bengali chars
- Never mix: verbatim source text is never edited to add coverage; self-written explanations always cover all 4

MATH/CHEMISTRY FORMATTING (always active, in question/options/explanation):
- Use proper Unicode subscript/superscript — never raw underscore/caret or plain digits where sub/superscript belongs
...
- Apply consistently across question, all 4 options, and explanation — never mix formats within one MCQ
...
Never use phrases referring back to the source itself instead of stating facts directly:
❌ "উল্লেখিত চিত্রে"/"চিত্রে দেখা যাচ্ছে"/"বক্সে"/"ছকে"/"উদ্দীপকে"/"সারণিতে"/"টপিকে"/"পৃষ্ঠায়"/"প্যাসেজে"/"অনুচ্ছেদে"/"গ্রাফে"
❌ "দেখা যাচ্ছে"/"বলা আছে"/"উল্লেখ করা আছে"/"লক্ষ করা যায়"/"দেখানো হয়েছে"/"দেওয়া আছে"/"প্রদত্ত"
...
async def _qbm_extract_from_image(img) -> list:
    ...
    genuinely has no MCQ". So one independent final-scan call is run before
    confirming empty, and the whole pipeline retries up to 2 more times (3
    total attempts) before a page is ever confirmed empty.
    """
...
def _qbm_restore_opt_bboxes(source_mcqs: list, verified_mcqs: list) -> list:
    """Call3-verify's own prompt schema doesn't ask for qsn_bbox, so its
    output would silently lose it. Verify keeps the same length/order as
...
async def _attach_option_images_if_missing(mcqs: list, img) -> list:
    """/qbm: if a source MCQ's QUESTION part has a diagram/figure (per qsn_bbox
    from the extraction prompt), tightly crop JUST that diagram, upload it,
    and embed <img src="..."> into the question text so it flows through to
    CSV/DB as-is. Options never get images."""
    try:
        from pdf_handler import crop_option_image
    except Exception:
        return mcqs
    for mc in mcqs or []:
        bbox = mc.get("qsn_bbox")
        q = mc.get("question") or ""
        if not bbox or "<img" in q.lower():
            continue
        try:
            url = await asyncio.to_thread(crop_option_image, img, bbox)
        except Exception as e:
            logger.warning(f"[QBM QsnImage] crop/upload failed: {e}")
            url = ""
        if url:
            mc["question"] = f'{q} <img src="{url}">'.strip()
    return mcqs
...
async def handle_auto_command(msg: dict):
    ...
    _tz = timezone(timedelta(hours=6))  # Bangladesh time
    ...
    def _fmt_clock(ts):
        return datetime.fromtimestamp(ts, _tz).strftime("%I:%M:%S %p")
    ...
    def _render_status(current_line, pct):
        ...
        lines_out = [
            f"🕐 শুরু হয়েছে: {_fmt_clock(_start_time)}",
            f"⏱️ চলছে: {_fmt_secs(elapsed)}",
        ]
        if pct and pct > 0:
            eta = (elapsed / pct) * (100 - pct)
            lines_out.append(f"⏳ আনুমানিক বাকি: {_fmt_secs(eta)}")
            lines_out.append(f"🏁 আনুমানিক শেষ হবে: {_fmt_clock(time.time() + eta)}")
...
def _onu_mcq_has_image(m: dict) -> bool:
    """True if this MCQ has any image attached — question diagram (qsn_bbox),
...
async def handle_message(msg: dict):
    ...
    if text.startswith("/permit"):
        ...
        if msg.get("from", {}).get("id") == OWNER_ID:
            await show_special_main_menu(OWNER_ID)
            if chat_id != OWNER_ID:
        ...
        _lines = [l.strip() for l in text.splitlines() if "t.me/" in l]
        _group_ref = _lines[0] if _lines else ""
        _spawn_command_task(uid, handle_ok_topic_range(msg, _group_ref, _start_n, _end_n))
