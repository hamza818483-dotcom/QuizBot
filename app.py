...
async def handle_wm_command(msg: dict):
    """/wm — 2 alada kaj kore:
    1) PDF-e reply diye /wm (name) dile -> shudhu diagonal watermark (page bg-e
       hালka tilted text) apply hoy sheI PDF-e. Footer change hoy na.
    2) Reply chara /wm (text) DM-e dile -> active footer text set hoy (porborti
       shob generated PDF-e footer hisebe bosbe). History theke pick korar
       jonno inline button list o dekhano hoy jokhon /wm shudhu (text chara) dawa hoy."""
    chat_id = msg["chat"]["id"]
    uid = msg["from"]["id"]
    text = msg.get("text", "").strip()
    ...
    if not wm_text:
        ...
        await send_msg(chat_id,
            "📌 Usage:\n"
            "<code>/wm YourName</code> — PDF-e reply করে দিলে সেটাতে diagonal watermark বসবে\n"
            "অথবা reply ছাড়া <code>/wm Footer Text</code> দিলে active footer text set হবে\n\n"
    ...
    # Case 1: PDF-e reply -> shudhu diagonal watermark apply, footer touch kora hoy na
    ...
    # Case 2: Reply chara -> active footer text set + history te save
...
TOPIC_EXTRACT_PROMPT = QBM_EXTRACT_PROMPT_DEFAULT.replace(
    ...
    '  b4) SPECIFIC TRAP — when a full-page-width banner sits at roughly the same vertical height as the LAST few MCQs of the right column (not clearly above or below all of them), check each of those right-column MCQs individually: if its own question/option text block is physically ABOVE the banner\'s top edge in the right column, it belongs to the OLD topic, even if the banner line, read left-to-right across the full page, happens to sit between the left column\'s last MCQ and the right column\'s last few MCQs. Never assign the new topic_hint to a right-column MCQ just because the banner is "in that general area" of the page — verify its box is strictly below the banner\'s own top edge within that same column.\n'
...
async def handle_ai(msg: dict):
    ...
    try:
        ...
        def _ai_progress(done: int):
            if not status_msg_id:
                return
            pct = int(done * 100 / _total_mcqs) if _total_mcqs else 100
            if pct - _last_shown["pct"] < 10 and pct != 100:
                return
            _last_shown["pct"] = pct
            bar = "█" * (pct // 10) + "░" * (10 - pct // 10)
...
async def _handle_qbm_impl(msg: dict):
    ...
    _is_cache_hit = (not is_image_reply) and (
    ...
    status_r = await send_msg(chat_id, ("✅ Cached PDF ব্যবহার হচ্ছে...\n📄 " + file_name) if _is_cache_hit else
        ("⏳ " + ("Image" if is_image_reply else "PDF") + f" download হচ্ছে...\n📄 {file_name}\n[░░░░░░░░░░ 0%]"))
    status_msg_id = status_r.get("result", {}).get("message_id")
