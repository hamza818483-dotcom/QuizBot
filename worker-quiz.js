export default {
  async fetch(request, env) {
    ...
    const DB = env.DB;
    const QS = env.QUIZ_SESSIONS;
    const BT = env.BOT_TOKEN;
    ...
    if (url.pathname === '/webhook') {
      ...
      if (up.callback_query) return cb(up.callback_query, DB, TG);
...
async function cb(q, DB, TG) {
  const d = q.data, cid = q.message.chat.id;
  if (d.startsWith('lb_')) { const lb = await DB.prepare('SELECT user_name, score FROM quiz_leaderboard WHERE quiz_id=?1 ORDER BY CAST(SUBSTR(score,1,INSTR(score,"/")-1) AS INTEGER) DESC LIMIT 10').bind(d.replace('lb_', '')).all(); let t = '🏆 *Leaderboard*\n\n'; const m = ['🥇', '🥈', '🥉']; (lb.results || []).forEach((r, i) => t += (m[i] || (i + 1 + '.')) + ' ' + r.user_name + ' — ' + r.score + '\n'); await fetch(TG + '/sendMessage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: cid, text: t || 'No data', parse_mode: 'Markdown' }) }); }
  if (d.startsWith('hist_')) { const hist = await DB.prepare('SELECT score, attempt FROM quiz_results WHERE user_id=?1 AND quiz_id=?2 ORDER BY attempt').bind(q.from.id, d.replace('hist_', '')).all(); let t = '📈 *Progress*\n\n'; (hist.results || []).forEach(r => t += '🟢 Attempt ' + r.attempt + ': ' + r.score + '\n'); await fetch(TG + '/sendMessage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: cid, text: t || 'No data', parse_mode: 'Markdown' }) }); }
  return new Response('OK');
}
