...
QUIZ_SESSIONS = {}
LEADERBOARD = {}  # {quiz_id: [{name, score, right, total}]}
HISTORY = {}  # {user_id_quiz_id: {right, score, pct}}

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if args and args[0].startswith('qz_'):
        db = await aiosqlite.connect('/data/data/com.termux/files/home/AtlasMasterBot/data/atlas_bot.db')
        q = await db.execute_fetchall('SELECT * FROM quizzes WHERE id=?', (args[0],))
        await db.close()
        if q:
            quiz = q[0]
            qs = json.loads(quiz[5])
            if quiz[4]:
                random.shuffle(qs)
                for x in qs:
            ...
            s = {'qid': args[0], 'name': quiz[1], 'qs': qs, 'cur': 0, 'tot': len(qs), 'right': 0, 'wrong': 0, 'skip': 0, 'timer': quiz[3] or 15, 'tag': quiz[6] or '', 'exp': quiz[7] or '', 'chat': update.effective_chat.id, 'uname': update.effective_user.first_name or 'Student', 'pid': None, 'cor': None}
            QUIZ_SESSIONS[update.effective_user.id] = s
            await update.message.reply_text(f"📝 *{quiz[1]}*\n⏳ Starting...", parse_mode='Markdown')
