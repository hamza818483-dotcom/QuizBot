...
কী টেস্ট করে:
1. gemini-3.5-flash vs gemini-3.5-flash — দুই model দিয়ে একই সহজ prompt,
...
- যদি gemini-3.5-flash consistently দ্রুত আর gemini-3.5-flash consistently
...
async def main():
    ...
    tests = [
        ("gemini-3.5-flash", SHORT_PROMPT, None, "2.5-flash + short prompt (no image)"),
        ("gemini-3.5-flash", SHORT_PROMPT, None, "3.6-flash + short prompt (no image)"),
        ("gemini-3.5-flash", SHORT_PROMPT, img_b64, "2.5-flash + short prompt + image"),
        ("gemini-3.5-flash", SHORT_PROMPT, img_b64, "3.6-flash + short prompt + image"),
        ("gemini-3.5-flash", LONG_PROMPT, img_b64, "2.5-flash + LONG bot-style prompt + image"),
        ("gemini-3.5-flash", LONG_PROMPT, img_b64, "3.6-flash + LONG bot-style prompt + image"),
    ]
    ...
    if fast_25 and fast_36:
        ...
        print(f"গড় সময় — gemini-3.5-flash: {avg25:.2f}s | gemini-3.5-flash: {avg36:.2f}s")
        if avg36 > avg25 * 1.5:
