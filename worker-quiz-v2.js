...
async function cb(q, DB, QS, TG) {
  ...
  if (d.startsWith('hist_')) { const uid = q.from.id, qid = d.replace('hist_', ''); const prev = await QS.get('prev_' + uid + '_' + qid); let t = '📈 *Progress*\n\n'; if (prev) { const p = JSON.parse(prev); t += '🟢 Latest: ' + p.score + ' (' + p.pct + '%)\n'; } else { t += 'No history yet.'; } await fetch(TG + '/sendMessage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: cid, text: t, parse_mode: 'Markdown' }) }); }
  return new Response('OK');
}
